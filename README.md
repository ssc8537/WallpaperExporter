...
- Scene/Web 未生成 Themes 快照时，程序会短暂显示纯桌面、隐藏图标与任务栏并抓取 Wallpaper Engine 实际渲染帧，随后立即恢复窗口。
...
- 当前页提供上一张和下一张；当前项目的 Steam 管理入口集中在“桌面全局快捷键”页面。
...
- 上一张只使用本次程序运行中确认过的导航记录，并将 Scene 路径转换为 `project.json` 后再交给 Wallpaper Engine，避免桌面变空。
...
- Scene/Web 没有现成最终图片；优先使用 Wallpaper Engine 新快照，未生成时按显示器真实分辨率抓取实际桌面渲染帧。
...
- 每个项目都必须确认 Wallpaper Engine 当前配置已切换到对应 Workshop ID；之后使用新快照或抓取实际渲染帧，不会沿用上一张图。
